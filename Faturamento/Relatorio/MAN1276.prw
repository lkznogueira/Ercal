...
/*/
+---------------------------------------------------------------------------+
| Programa  ³ MAN1276  º Autor ³ Claudio            º Data ³  16/03/2012    |
+---------------------------------------------------------------------------+
...
User Function MAN1276()                    
	...
	cString  :="SRA"
	lEnd     :=.F.
	cDesc1   := "Este relatorio ira imprimir a CARTA DE CORREÇÃO ELETRONICA (CC-e)"
	cDesc2   := ""
	cDesc3   := ""
	cPerg    := ""
	nomeprog := "MAN1276"
	...
	Processa({|| Gera1276()},"Gera Dados para impressão")  
	If !Empty(aResult[1])
		RptStatus({|| Imp1276()},"Imprimindo Relatório")
	Else
		MsgInfo("DANFE não tem carta de correção... VERIFIQUE!!") 
	EndIf	
...
Static Function Gera1276()
	...
	// verifica se existe a SP, e se não cria
	...
	IF TCSPExist("Pega_NFeId")
		...
		IF empty(aResult)
			MsgInfo("Erro na execução da Stored Procedure : "+TcSqlError()+" - Linha 68 - Avise ao Suporte Técnico" ) 
			Return
		EndIf	
	EndIf	
	...
	If !Empty(vNfeChv)
		IF TCSPExist("Dados_CCe")
			...
			IF empty(aResult)
				MsgInfo("Erro na execução da Stored Procedure : "+TcSqlError()+" - Linha 79 - Avise ao suporte Tecnico") 
				Return
			Endif	
		EndIf	
	EndIf	
...
//	oPrn:SetLandscape()
	...
	oPrn:Say(nLin ,0210 ,"CARTA DE CORREÇÃO", 		oFta14N, 100 )                                  
	nlin+=40                        	
	...
	oPrn:Say(nLin ,0420 ,"Orgão", 		oFta12, 100 )                                  
	nlin+=10                        	
	...
	oPrn:Say(nLin ,0420 ,"Versão Evento",	oFta12, 100 )                                  
	nlin+=10                        	
	...
	oPrn:Say(nLin ,0010 ,"INFORMAÇÕES DA CARTA DE CORREÇÃO",		oFta12N, 100 )                                  
	nlin+=40                        	
	...
	oPrn:Say(nLin ,0010 ,"Texto da Carta de Correção",			oFta12, 100 )                                  
	nlin+=10                        	
	...
	oPrn:Say(nLin ,0010 ,"Condições de Uso da Carta de Correção",			oFta12, 100 )                                  
	nlin+=10                        	
...
/* 
AS ROTINAS ABAIXO SÃO UTILIZADAS ACIMA, PARA FORMATAR OS TEXTOS DA CCE.
...
PS: ESTAS ROTINAS NÃO SÃO MINHAS. UMA ALMA CARIDOSA ESCREVEU EM CLIPPER E DISPONIBILIZOU NA INTERNET
