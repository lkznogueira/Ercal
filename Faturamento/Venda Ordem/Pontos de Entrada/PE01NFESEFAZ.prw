...
User Function PE01NFESEFAZ()
	...
	cQuery += "AND SC6.C6_CF NOT IN ('5923','6923')"
	Count To nLinha
	...
	If cTIPc == '1'// .And. cEST <>'MG'
		if (cGrup == '0056' .OR. cGrup == '0106') .And. cEST <> SuperGetMV("MV_ESTADO") //.OR. cGrup == '0106'
			If cEmpAnt <> '50' //pega mensagem padrao sistema
		...
		EndIF
	EndIF
	If QADB->C6_NOTA <> ' ' //QADB->(LASTREC()) > 0 //+ ALLTRIM(SF2->F2_VEND1) + " - " + ALLTRIM(cNOMEVEND)+" - NF-eMãe:"+ QADB->C6_NOTA+";"
		_cRet += " Rep.: " + ALLTRIM(cNOMEVEND)+" - NFe Mãe:"+ QADB->C6_NOTA+Space(1)//";"
	ELSE
		_cRet += " Rep.: " +ALLTRIM(cNOMEVEND)+Space(1)//" - NFe Mãe:"+ QADB->C6_NOTA+";"
	EndIF
	...
	IF !Empty(cGprod)
		...
		EndIf
	EndIF
	_cRet += "PLACA: "+Posicione("DA3",1,XFILIAL("DA3")+SC5->C5_VEICULO,"DA3_PLACA")
	cMensCli += _cRet
