		...
		IF cGrup == '0056' .OR.  cGrup == '0106' .OR. cGrup == '0108'
			...
			Else //CALCULO BRITACAL
				If trim(cFilAnt) == '2000' //se Calta TO
					...
					nDESC := 0.92799 //0.88
					nFatConv := 7.76 //fator conv para achar aumento
					nVlrau := ROUND((aCols[n,7]*nFatConv)/100,2) //valor aumentado
					nVlrUn := ROUND(((aCols[n,7]+nVlrau)/aCols[N][nPQuant]),4)
					vDesc := nVlrau//ROUND(aCols[n,7]-(aCols[n,7]*nDESC),2) //ROUND((aCols[n,7]-(aCols[n,7]*nDESC)),4)//ROUND(((aCols[n,7] - ((aCols[n,7]*vIcm)/100))*nDESC/100),2)
					
					MsgInfo("CONTRIBUINTE FORA DO ESTADO, TERA DESCONTO DE R$: "+Alltrim(Transform(vDesc," @E 999,999.99"))+" na Nota fiscal!!!")
