...
Static Function eventoNFE(_cAcao,_lYesNo)
	...
	If _lYesNo
		...
		If _cAcao <> "Transmissao" .And. FWAlertYesNo('Confirma a EXCLUSรO da NOTA FISCAL?', 'ATENวรO')
			lExclui := .T.
			//ฆ+---------------------------------------------------------+ฆ
...
Static Function BuscaCarga(cNumOrdem,_xcNopc)
	...
	if ZC3->(DbSeek(xFilial("ZC3")+cNumOrdem))
		...
		Else
			FWAlertError("Nao foi possํvel posicionar no contrato!")
			Return()
		EndIf
...
Static Function GeraPedido(cOrdem,cCodPlaca,cNumPCom)
	...
	//ฆ+---------------------------------------------------------+ฆ
	//ฆฆValida Venda para Industrializa็ใo
	//ฆ+---------------------------------------------------------+ฆ
	If ADB->ADB_XTIPO == "09"
