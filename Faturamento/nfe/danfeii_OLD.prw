...
If Type("oIdent:_tpAmb:TEXT")<>"U" .And. oIdent:_tpAmb:TEXT=="2"
	...
	EndDo
	*/
	// Ercal - quebra de linha nas informações adicionais. Adicionar ponto e vírgula nas mensagens - Carlos Daniel
	While !Empty(cAux)
		_nQuebra := At(";",cAux)
		cAux2    := If(_nQuebra > 0,  Left(cAux,_nQuebra), cAux)
		While !Empty(cAux2)
		...
		_nAt :=  At(";",cAux)
		cAux := If(_nAt == 0,Alltrim(cAux),SubStr(cAux,_nAt+1))
		
		If Len(aMensagem) > 0
			If At(";",aMensagem[Len(aMensagem)]) > 0
				aMensagem[Len(aMensagem)] := Alltrim(StrTran(aMensagem[Len(aMensagem)],";","."))
			EndIf
		EndIf
		cAux := If(_nAt == 0, "", cAux)
	EndDo
