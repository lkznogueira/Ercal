...
//DEFINE MSDIALOG _opDlgPcp TITLE "Nf-e Disponíveis" From 000,000 to nlTl3,nlTl4 PIXEL
...
olBDsfz  := TButton():New(nlTl1+132,nlTl2+065,"Desfazer amarração do produto",_opSPeDlg, {|| Iif(Aviso("Deseja desfazer a amarração?",("Ao clicar em <SIM> "+CRLF+"A amarração do pedido será excluída."),{"Sim","Nâo"})==1   											,;  // Condicao
	...
	Else
		...
		If lProces
			...
			For nX := 1 To Len(aItens)
				...
				Else  
					...
					Else  */
					   //RETIRADO AMARRACAO INICIAL PARA FICAR SOMENTE A FINAL	     
						...
						//MsgAlert('Atencao Produto '+cProduto+' não Encontrado na Amarraçâo Prod. x Fornecedor, Deseja incluir?' )
						   ...
						   /*	@ 65,153 To 229,435 Dialog oDlg Title OemToAnsi("Inclusão Amarração")
							...
							@ 9,70 Say OemToAnsi("Descrição") Size 99,8
							@ 28,70 Get cDesc Picture "@E"  Size 59,10
							@ 62,39 BMPBUTTON TYPE 1 ACTION Close(oDlg)
							
							DEFINE MSDIALOG oDlg TITLE OemToAnsi("Amarração de Produtos") FROM 000, 000  TO 300, 500 COLORS 0, 16777215 PIXEL
						
							    @ 007, 043 SAY oSay1 PROMPT OemToAnsi("Amarração Produto X Fornecedor") SIZE 167, 015 OF oDlg FONT oFont1 COLORS 16711680, 16777215 PIXEL
							    ...
							    @ 050, 083 SAY oSay4 PROMPT "Descrição Fornecedor" SIZE 060, 007 OF oDlg COLORS 0, 16777215 PIXEL
							...
							If !Empty(cProduto2)
						   	...
						   	Else
								...
								MsgAlert("As amarrações desta nota deverao ser efetuadas posteriormente para geração da pré nota de entrada")
								Exit
							EndIf
				...
				{"DT_VUNIT"		,Round((Val(aItens[nX]:_Prod:_vUnCom:Text))/(_nConv),TamSX3("C7_PRECO")[2])	},; //Vlor Unitário
